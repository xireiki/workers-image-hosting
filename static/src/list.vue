              ...
              if (lyricsText) {
                ...
                if (lyricsText.includes('[') && /\[\d{2}:\d{2}/.test(lyricsText)) {
                  ...
                  lyricsEl.innerHTML = lyricsData.map((line, index) => {
                    ...
                    // 主歌词
                    html += `<p class="lyric-main" data-time="${line.time}" style="margin:0;opacity:0.5;transition:all 0.6s ease;font-size:16px;line-height:1.4;">${line.text || '&nbsp;'}</p>`;
                    
                    // 翻译（如果有）
                    if (line.translation) {
                      html += `<p class="lyric-trans" style="margin:1px 0 0 0;opacity:0.5;transition:all 0.6s ease;font-size:14px;line-height:1.4;">${line.translation}</p>`;
                    }
